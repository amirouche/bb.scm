(library (bb cli)
  ...
  (define print-usage
    (lambda ()
      ...
      (display "  bb commit [<ref>... | --all]            Promote staged combiners to committed\n")
    ...
    (lambda (arguments)
      (let* ((root (store-find-root (current-directory)))
        ...
        (let ((count 0))
          (for-each
           (lambda (name)
             (let ((fhash (guard (exn (#t #f))
                            (let-values (((h _l _m) (resolve-ref name-index root name)))
                              h))))
               (if fhash
                   (let* ((fhash fhash)
    ...
    (lambda ()
      ...
      (let* ((temp-dir (format #f "/tmp/bb-test-print-~a" (random 1000000)))
        ...
        (let ((dir-h1  (store-path-join temp-dir "combiners" hash-h1))
          ...
          (let ((map-h1  (store-path-join dir-h1  "mappings" "01"))
                (map-h2  (store-path-join dir-h2  "mappings" "02"))
                (map-app (store-path-join dir-app "mappings" "03")))
      ...
      (let* ((temp-dir (format #f "/tmp/bb-test-print-~a" (random 1000000)))
        ...
        (let ((dir-add1 (store-path-join temp-dir "combiners" hash-add1))
          ...
          (let ((map-add1 (store-path-join dir-add1 "mappings" "01"))
                (map-dbl  (store-path-join dir-dbl  "mappings" "02")))
